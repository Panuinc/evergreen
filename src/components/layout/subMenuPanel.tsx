...
export default function SubMenuPanel({ activeMenu, isCollapsed, onSubMenuClick }: SubMenuPanelProps) {
        ...
        {activeMenu.subMenus?.map((subMenu, index) => {
          ...
          return (
            ...
            >
              ...
              {badgeCount > 0 && (
                <Chip size="md" color="danger" variant="solid" className="ml-auto min-w-6 h-5 text-xs">
                  {badgeCount}
                </Chip>
              )}
            </Link>
