...
- **ลูกค้า (bcCustomer)**: bcCustomerNo, bcCustomerNameValue, bcCustomerPhoneNo, bcCustomerContact, bcCustomerBalanceLCY, bcCustomerBalanceDueLCY, bcCustomerSalespersonCode
- **สินค้า (bcItem)**: bcItemNo, bcItemDescription, bcItemType, bcItemInventory, bcItemUnitPrice, bcItemUnitCost, bcItemItemCategoryCode, bcItemBaseUnitOfMeasure, bcItemBlocked
- **ใบสั่งขาย (bcSalesOrder)**: bcSalesOrderNoValue, bcSalesOrderSellToCustomerName, bcSalesOrderOrderDate, bcSalesOrderDueDate, bcSalesOrderStatus, bcSalesOrderCompletelyShipped, bcSalesOrderSalespersonCode, bcSalesOrderExternalDocumentNo, bcSalesOrderAmountIncludingVAT
- **รายการสินค้าในใบสั่งขาย (bcSalesOrderLine)**: bcSalesOrderLineDescriptionValue, bcSalesOrderLineQuantityValue, bcSalesOrderLineUnitPrice, bcSalesOrderLineAmountValue, bcSalesOrderLineQuantityShipped, bcSalesOrderLineUnitOfMeasureCode
...
- **bcCustomerBalanceLCY** = ยอดค้างชำระทั้งหมด, **bcCustomerBalanceDueLCY** = ยอดที่เกินกำหนด
- **externalDocumentNo** = เลข PO หรือเลขอ้างอิงของลูกค้า
...
- "ลูกค้า X สั่งอะไรบ้าง" → get_customers(search:"X") เพื่อหา customerNo แล้ว get_sales_orders(customerNumber)
...
async function executeTool(name, args) {
  ...
  switch (name) {
    case "get_customers": {
      let q = supabase
        .from("bcCustomer")
        .select("bcCustomerNo,bcCustomerNameValue,bcCustomerPhoneNo,bcCustomerContact,bcCustomerBalanceLCY,bcCustomerBalanceDueLCY,bcCustomerSalespersonCode")
        .order("bcCustomerNameValue")
        .limit(args.limit || 100);
      if (args.search) q = q.ilike("bcCustomerNameValue", `%${args.search}%`);
    ...
    case "get_items": {
      let q = supabase
        .from("bcItem")
        .select("bcItemNo,bcItemDescription,bcItemType,bcItemInventory,bcItemUnitPrice,bcItemUnitCost,bcItemItemCategoryCode,bcItemBaseUnitOfMeasure,bcItemBlocked")
        .eq("bcItemBlocked", false)
        .order("bcItemNo")
        .limit(args.limit || 100);
      if (args.search) {
        q = q.or(`bcItemNo.ilike.%${args.search}%,bcItemDescription.ilike.%${args.search}%`);
      }
      if (args.category) q = q.eq("bcItemItemCategoryCode", args.category);
    ...
    case "get_sales_orders": {
      let q = supabase
        .from("bcSalesOrder")
        .select("bcSalesOrderNoValue,bcSalesOrderSellToCustomerNo,bcSalesOrderSellToCustomerName,bcSalesOrderOrderDate,bcSalesOrderDueDate,bcSalesOrderStatus,bcSalesOrderCompletelyShipped,bcSalesOrderSalespersonCode,bcSalesOrderExternalDocumentNo,bcSalesOrderAmountIncludingVAT")
      ...
      if (args.customerNumber) q = q.eq("bcSalesOrderSellToCustomerNo", args.customerNumber);
      if (args.since) q = q.gte("bcSalesOrderOrderDate", args.since);
      const { data: orders, error: oErr } = await q;
      if (oErr) throw new Error(oErr.message);

      if (!orders?.length) return [];

      const orderNos = orders.map((o) => o.bcSalesOrderNoValue);
      const { data: lines, error: lErr } = await supabase
        .from("bcSalesOrderLine")
        .select("bcSalesOrderLineDocumentNo,bcSalesOrderLineNoValue,bcSalesOrderLineDescriptionValue,bcSalesOrderLineQuantityValue,bcSalesOrderLineUnitPrice,bcSalesOrderLineAmountValue,bcSalesOrderLineQuantityShipped,bcSalesOrderLineUnitOfMeasureCode")
      ...
      for (const l of lines || []) {
        ...
        linesByOrder[l.bcSalesOrderLineDocumentNo].push({
          ...
          lineAmount: l.bcSalesOrderLineAmountValue,
          quantityShipped: l.bcSalesOrderLineQuantityShipped,
          uom: l.bcSalesOrderLineUnitOfMeasureCode,
        });
      }
      return orders.map((o) => ({ ...o, lines: linesByOrder[o.bcSalesOrderNoValue] || [] }));
    }

    case "get_sales_summary": {
      let q = supabase
        .from("bcSalesOrder")
        .select("bcSalesOrderNoValue,bcSalesOrderSellToCustomerNo,bcSalesOrderSellToCustomerName,bcSalesOrderOrderDate,bcSalesOrderStatus,bcSalesOrderCompletelyShipped,bcSalesOrderSalespersonCode,bcSalesOrderAmountIncludingVAT");
      if (args.since) q = q.gte("bcSalesOrderOrderDate", args.since);
      if (args.until) q = q.lte("bcSalesOrderOrderDate", args.until);
      if (args.salespersonCode) q = q.eq("bcSalesOrderSalespersonCode", args.salespersonCode);
      const { data: orders, error } = await q;
      if (error) throw new Error(error.message);

      const rows = orders || [];
      const totalRevenue = rows.reduce((s, o) => s + (o.bcSalesOrderAmountIncludingVAT || 0), 0);
      const result = {
      ...
      if (args.groupBy === "salesperson") {
        ...
        for (const o of rows) {
          ...
          groups[key].orders++;
          groups[key].revenue += o.bcSalesOrderAmountIncludingVAT || 0;
        }
        result.byGroup = Object.values(groups).sort((a, b) => b.revenue - a.revenue);
      } else if (args.groupBy === "customer") {
        const groups = {};
        for (const o of rows) {
          const key = o.bcSalesOrderSellToCustomerNo;
          if (!groups[key]) groups[key] = { customerNumber: key, customerName: o.bcSalesOrderSellToCustomerName, orders: 0, revenue: 0 };
          groups[key].orders++;
          groups[key].revenue += o.bcSalesOrderAmountIncludingVAT || 0;
        }
        ...
        const groups = {};
        for (const o of rows) {
          const month = o.bcSalesOrderOrderDate ? o.bcSalesOrderOrderDate.slice(0, 7) : "ไม่ระบุ";
          if (!groups[month]) groups[month] = { month, orders: 0, revenue: 0 };
          groups[month].orders++;
          groups[month].revenue += o.bcSalesOrderAmountIncludingVAT || 0;
        }
