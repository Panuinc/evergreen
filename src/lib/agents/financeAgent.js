...
- **หนี้ลูกค้า (bcCustomer)**: balanceLCY = ยอดค้างชำระทั้งหมด, balanceDueLCY = ยอดที่เกินกำหนดชำระ
- **ออเดอร์ค้างส่ง**: ออเดอร์ที่ completelyShipped=false พร้อมมูลค่า

## ความรู้เฉพาะทาง
- **balanceLCY** = ยอดคงค้างทั้งหมดในระบบ BC (รวมยังไม่ถึงกำหนด)
- **balanceDueLCY** = ยอดที่ **เกินกำหนดชำระแล้ว** (ต้องติดตามด่วน)
- **amountIncludingVAT** = ยอดขายรวม VAT
...
- "ลูกค้าค้างชำระ" → get_customer_balances(hasDebt: true, sortBy: "bcCustomerBalanceDueLCY")
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          sortBy: {
            type: "string",
            enum: ["bcCustomerBalanceDueLCY", "bcCustomerBalanceLCY", "bcCustomerNameValue"],
            description: "เรียงตาม: bcCustomerBalanceDueLCY (เกินกำหนด), bcCustomerBalanceLCY (ค้างทั้งหมด), bcCustomerNameValue (ชื่อ) — default: bcCustomerBalanceDueLCY",
          },
          limit: { type: "number", description: "จำนวนสูงสุด (default: 20)" },
          hasDebt: { type: "boolean", description: "true = แสดงเฉพาะลูกค้าที่มียอดค้างชำระ (bcCustomerBalanceDueLCY > 0)" },
        },
      },
    },
  },
...
async function executeTool(name, args) {
  ...
  switch (name) {
    case "get_revenue_summary": {
      ...
      const orders = rows || [];
      const totalRevenue = orders.reduce((s, o) => s + (o.bcSalesOrderAmountIncludingVAT || 0), 0);
      const result = {
        ...
        pendingRevenue: orders.filter((o) => !o.bcSalesOrderCompletelyShipped).reduce((s, o) => s + (o.bcSalesOrderAmountIncludingVAT || 0), 0),
      };
      ...
      if (args.groupBy === "salesperson") {
        ...
        for (const o of orders) {
          ...
          g[k].orders++;
          g[k].revenue += o.bcSalesOrderAmountIncludingVAT || 0;
        }
        result.byGroup = Object.values(g).sort((a, b) => b.revenue - a.revenue);
      } else if (args.groupBy === "customer") {
        const g = {};
        for (const o of orders) {
          const k = o.bcSalesOrderSellToCustomerNo;
          if (!g[k]) g[k] = { customerNumber: k, customerName: o.bcSalesOrderSellToCustomerName, orders: 0, revenue: 0 };
          g[k].orders++;
          g[k].revenue += o.bcSalesOrderAmountIncludingVAT || 0;
        }
        ...
        const g = {};
        for (const o of orders) {
          const k = o.bcSalesOrderOrderDate ? o.bcSalesOrderOrderDate.slice(0, 7) : "ไม่ระบุ";
    ...
    case "get_customer_balances": {
      const sortCol = args.sortBy || "bcCustomerBalanceDueLCY";
      let q = supabase
        .from("bcCustomer")
        .select("bcCustomerNo,bcCustomerNameValue,bcCustomerPhoneNo,bcCustomerBalanceLCY,bcCustomerBalanceDueLCY,bcCustomerSalespersonCode")
        .order(sortCol, { ascending: false })
        .limit(args.limit || 20);
      if (args.search) q = q.ilike("bcCustomerNameValue", `%${args.search}%`);
      if (args.hasDebt) q = q.gt("bcCustomerBalanceDueLCY", 0);
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      const rows = data || [];
      return {
        count: rows.length,
        totalBalance: rows.reduce((s, c) => s + (c.bcCustomerBalanceLCY || 0), 0),
        totalBalanceDue: rows.reduce((s, c) => s + (c.bcCustomerBalanceDueLCY || 0), 0),
        customers: rows,
      };
    }
    ...
    case "get_outstanding_orders": {
      let q = supabase
        .from("bcSalesOrder")
        .select("bcSalesOrderNoValue,bcSalesOrderSellToCustomerNo,bcSalesOrderSellToCustomerName,bcSalesOrderOrderDate,bcSalesOrderDueDate,bcSalesOrderStatus,bcSalesOrderSalespersonCode,bcSalesOrderAmountIncludingVAT")
      ...
      return {
        count: rows.length,
        totalValue: rows.reduce((s, o) => s + (o.bcSalesOrderAmountIncludingVAT || 0), 0),
        orders: rows,
      };
    }
