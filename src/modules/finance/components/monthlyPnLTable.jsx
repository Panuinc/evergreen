...
import { calMonths, calMonthsShort, calMonthBE } from "@/lib/glAccountMap";
...
export default function MonthlyPnLTable({ data, chartData, loading, year, compYears = [] }) {
  ...
  const handleExport = () => {
    const columns = [
      ...
      ...calMonths.map((m, i) => ({
        header: `${calMonthsShort[i]} ${calMonthBE(i, year)}`,
        key: `m_${m}`,
        width: 15,
      })),
    ...
    const exportData = data
      ...
      .map((r) => {
        ...
        calMonths.forEach((m) => { row[`m_${m}`] = r.months?.[m] || 0; });
              ...
              <tr className="bg-default-100 border-b border-border">
                ...
                {calMonths.map((m, i) => (
                  <th key={m} className="text-right px-2 py-2 min-w-[90px] font-light">
                    {calMonthsShort[i]} {calMonthBE(i, year)}
                  </th>
                ))}
