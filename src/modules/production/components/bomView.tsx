...
const gridNumbers = [1, 2, 3, 4, 5, 6, 7, 8];
...
const LockBlockSVG = memo(({ x, y, width, height }: { x: number; y: number; width: number; height: number }) => (
...
const EnhancedEngineeringDrawing = memo(
  ({ results, coreCalculation, surfaceMaterial }: { results: any; coreCalculation: any; surfaceMaterial: any }) => {
    ...
    const piecesPerSide = parseInt(String(lockBlockCount / (lockBlockSides || 1))) || 0;

    const dims = useMemo(
      () => ({
        front: {
          W: safeW * drawingScale,
          H: safeH * drawingScale,
          F: safeF * drawingScale,
          DF: drawingDF * drawingScale,
          totalFrame: safeF * drawingScale,
          R: safeR * drawingScale,
          lockBlockW: safeF * drawingScale,
        },
        side: {
          T: safeT * drawingScale,
          H: safeH * drawingScale,
          S: safeS * drawingScale,
        },
        exterior: {
          W: safeW * drawingScale,
          H: safeH * drawingScale,
    ...
    const toggleAllLayers = useCallback((visible) => {
      setVisibleLayers(
        Object.fromEntries(
          Object.keys(layerConfig).map((key) => [key, visible]),
        ),
      );
    }, []);
    ...
    const renderLockBlocks = useCallback(() => {
      const blocks = [];
      const lockBlockH = lockBlockHeight * drawingScale;
      const lockBlockY =
        positions.front.y + dims.front.H - lockBlockBottom * drawingScale;
    ...
    const renderRails = useCallback(() => {
      ...
      return railPositions.map((pos, idx) => {
        const railY = positions.front.y + dims.front.H - pos * drawingScale;
        return (
          <FilledRect
            key={`front-rail-${idx}`}
        ...
        coreCalculation.pieces.forEach((piece, idx) => {
          elements.push(
            <rect
              ...
              x={positions.front.x + piece.x * drawingScale}
              y={positions.front.y + piece.y * drawingScale}
              width={piece.width * drawingScale}
              height={piece.height * drawingScale}
              fill={`url(#${pid})`}
              stroke="#000000"
              strokeWidth="0.5"
              strokeDasharray="4,4"
        ...
        coreCalculation.damPieces.forEach((p) => {
          elements.push(
            <rect
              key={p.id}
              className="layer-core"
              x={positions.front.x + p.x * drawingScale}
              y={positions.front.y + p.y * drawingScale}
              width={p.width * drawingScale}
              height={p.height * drawingScale}
              fill={`url(#${pid})`}
              stroke="#000000"
              strokeWidth="0.35"
            />,
    ...
    return (
      <div className="relative w-full h-full flex flex-col bg-default-100 rounded-lg overflow-hidden">
              ...
              <PopoverContent className="w-64">
                <div className="p-2 space-y-2">
                  <div className="flex justify-between items-center pb-2 border-b border-border gap-2">
                    <span className="font-light text-xs">เลเยอร์</span>
                      ...
                      {!noRailCoreTypes.includes(
                        ...
                        railPositions.map((pos, idx) => {
                          const railY =
                            ...
                            dims.side.H -
                            pos * drawingScale;
                          const railH = safeR * drawingScale * 0.5;
                          return (
                            <FilledRect
                              key={`side-rail-${idx}`}
                              className="layer-rails"
                      ...
                      {(lockBlockLeft || lockBlockRight) && (
                        <rect
                          ...
                          y={
                            positions.side.y +
                            dims.side.H -
                            lockBlockBottom * drawingScale
                          }
                          width={dims.side.T - 2 * dims.side.S}
                          height={lockBlockHeight * drawingScale}
                          fill="none"
                          stroke="#000000"
                          strokeWidth="1.4"
                          strokeDasharray="6,4"
                      ...
                      {(lockBlockLeft || lockBlockRight) &&
                        (() => {
                          ...
                          const lockBlockBottomY =
                            lockBlockTopY + lockBlockHeight * drawingScale;
                          return (
                            <DimLine
                              x1={positions.side.x}
                              y1={lockBlockTopY}
                      ...
                      {(lockBlockLeft || lockBlockRight) && (
                        <DimLine
                          ...
                          y2={
                            positions.front.y +
                            dims.front.H -
                            lockBlockPosition * drawingScale
                          }
        ...
        <div className="flex items-center justify-between p-2 bg-default-50 border-t border-border text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            <span>
              ประตู: {T}×{W}×{H} มม.
            </span>
            ...
            {!noRailCoreTypes.includes(coreCalculation?.coreType?.value) &&
...
const UIDoorBom = ({
  ...
  const isNoRailCoreType = noRailCoreTypes.includes(coreType);

  const [framePlanPage, setFramePlanPage] = useState(1);
  const [corePlanPage, setCorePlanPage] = useState(1);
  const framePageSize = 10;
  const corePageSize = 5;
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-3 w-full">
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">📝 ข้อมูลลูกค้า</span>
              </div>
            </CardHeader>
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">🎨 วัสดุผิว</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-2 text-xs p-2">
                ...
                <div className="flex justify-between">
                  <span>กาว:</span>
                  <span>
                    {glueThickness} มม. × 2 = {glueThickness * 2} มม.
                  </span>
                </div>
                <Divider className="" />
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">🪵 กรอบไม้ (ERP)</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
                ...
                frameLengthOptions?.recommendedOption && (
                  <div className="flex flex-col gap-2 p-3 bg-success-50 rounded-lg border border-success-200">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-light text-success-700">
              ...
              <div className="flex flex-col gap-2">
                <span className="text-xs font-light">ด้านที่ซ้อนกรอบ</span>
              ...
              >
                {doubleFrameCountOptions.map((opt) => (
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">➖ คานขวาง</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              {isNoRailCoreType ? (
                <div className="flex flex-col gap-2 text-xs p-2 bg-default-50 rounded-lg">
                  <Chip color="default" variant="flat" size="md">
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">🔒 บล็อกล็อค</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  {lockBlockPiecesOptions.map((opt) => (
                    <SelectItem key={opt.value}>{opt.label}</SelectItem>
                  ))}
                </Select>
                <Select
                ...
                <div className="flex flex-col gap-2 text-xs p-2 bg-default-50 rounded-lg">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-light text-foreground">
                      ...
                      {currentFrame.useWidth || 0}×{lockBlockHeight} mm
                    </span>
                  </div>
                  <span className="text-xs text-muted-foreground">
                    (ใช้ไม้เดียวกับกรอบ)
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">🧱 วัสดุไส้ประตู</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              <Select
              ...
              {selectedCoreItem && (
                <div className="flex flex-col gap-2 text-xs p-2 bg-primary-50 rounded-lg">
                  <div className="flex justify-between">
                    <span>รหัส:</span>
                    <span className="font-light text-foreground">
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">📐 การทำขอบประตู</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              <div className="flex items-center justify-between p-2">
                <span className="text-xs font-light">ทำขอบประตู</span>
                <Switch
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">🔩 เจาะใส่อุปกรณ์</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              <div className="flex items-center justify-between p-2">
                <span className="text-xs font-light">เจาะอุปกรณ์</span>
                <Switch
                  isSelected={drilling}
                  ...
                  {[
                    ...
                    >
                      <Switch
                        ...
                        size="md"
                      />
                      <span className="text-xs font-light min-w-[60px]">
                        {label}
                      </span>
                      {drillItems[key].checked && (
                        <Input
        ...
        <div className="xl:col-span-3 flex flex-col gap-3">
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">แบบวาด</span>
              </div>
            </CardHeader>
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-light text-xs">💰 สรุปราคา</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              <div className="flex flex-col gap-2 text-xs p-2 bg-default-50 rounded-lg">
          ...
          <Card shadow="none" className="w-full border border-border">
            <CardHeader className="border-b border-border bg-default-50">
              <div className="flex items-center gap-2">
                <span className="font-light text-xs">📋 สรุปโครงสร้าง</span>
              </div>
            </CardHeader>
            <CardBody className="gap-3 p-4">
              ...
              {doubleConfigSummary && (
                <div className="p-2 bg-default-50 rounded-lg text-xs text-foreground">
                  {doubleConfigSummary}
                </div>
              )}
              {selectedFrameCode && (
                <div className="p-2 bg-default-50 rounded-lg text-xs">
                  <span className="font-light text-foreground">
            ...
            <Card shadow="none" className="w-full border border-border">
              ...
              <CardBody className="gap-3 p-4">
                ...
                <div className="border-1 border-border rounded-lg overflow-hidden">
                  <div className="p-2 text-xs font-light bg-default-100">
                      ...
                      <div className="border-1 border-border rounded-lg overflow-hidden">
                        ...
                        {framePageCount > 1 && (
                          <div className="flex items-center justify-between px-3 py-2 border-t border-border bg-default-50">
                            ...
                            <span className="text-xs text-muted-foreground">
                              ท่อนที่ {(framePage - 1) * framePageSize + 1}–
                              {Math.min(
                                framePage * framePageSize,
                      ...
                      <div className="mt-3 border-1 border-border rounded-lg overflow-hidden">
                        ...
                        {corePageCount > 1 && (
                          <div className="flex items-center justify-between px-3 py-2 border-t border-border bg-default-50">
                            ...
                            <span className="text-xs text-muted-foreground">
                              แผ่นที่ {(corePage - 1) * corePageSize + 1}–
                              {Math.min(
                                corePage * corePageSize,
